			...
			<section>
				<div class="content-header">
					<div class="container-fluid">
						...
						<a href="/product/controller"class="btn bg-gra-03 p-2 mb-3 btn-xs pull-right"><b>+</b> Thêm mới sản phẩm </a>
							<tr>
								<th><a
									href="/product/list?field=id&amp;sort=${field}&amp;p=${page.number}">Mã sản phẩm</a></th>
								<th><a
									href="/product/list?field=image&amp;sort=${field}&amp;p=${page.number}">Ảnh</a></th>
								<th><a
									href="/product/list?field=name&amp;sort=${field}&amp;p=${page.number}">Tên sản phẩm</a></th>
								<th><a
									href="/product/list?field=price&amp;sort=${field}&amp;p=${page.number}">Giá sản phẩm</a></th>
								<th><a
									href="/product/list?field=sale&amp;sort=${field}&amp;p=${page.number}">Giảm giá</a></th>
								<th><a
									href="/product/list?field=createDate&amp;sort=${field}&amp;p=${page.number}">Ngày tạo</a></th>
								<th>Category ID</th>
							...
							<c:forEach var="item" items="${page.content}">
								<tr  style="  height: 30px; line-height: 30px;">
									<td>${item.id}</td>
									<td><img alt="" src="../views/image/${item.image}"
										style="width: 50px"></td>
									<td>${item.name}</td>
									<td>${item.sale}</td>
									<td>${item.price}</td>				
									...
									<td class="text-center"><a class='btn btn-info btn-xs'
										href="/product/edit/${item.id}"><i class="bi bi-pencil-square"></i> Sửa</a> <a
										href="/product/delete/${item.id}"
										class="btn btn-danger btn-xs"><i class="bi bi-x-lg"></i> Xóa</a></td>
								</tr>
							</c:forEach>
						</table>
						 <div class=" text-center">
              <a href="/product/list?p=0" class="btn btn-success">
                 First
              </a>
              <a href="/product/list?p=${page.number-1}" class="btn btn-primary">
                 Previous
              </a>
              <a href="/product/list?p=${page.number+1}" class="btn btn-danger">
                 Next
              </a>
              <a href="/product/list?p=${page.totalPages-1}" class="btn btn-warning">
								Last</a>
            </div>
